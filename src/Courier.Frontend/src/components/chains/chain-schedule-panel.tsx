...
import { useChainSchedules, useCreateChainSchedule, useUpdateChainSchedule, useDeleteChainSchedule } from "@/lib/hooks/use-chain-schedules";
import type { ChainScheduleDto, CreateChainScheduleRequest, UpdateChainScheduleRequest } from "@/lib/types";
...
interface ChainSchedulePanelProps {
  chainId: string;
}
...
export function ChainSchedulePanel({ chainId }: ChainSchedulePanelProps) {
  const { data, isLoading } = useChainSchedules(chainId);
  const createSchedule = useCreateChainSchedule(chainId);
  const updateSchedule = useUpdateChainSchedule(chainId);
  const deleteSchedule = useDeleteChainSchedule(chainId);

  const [showAdd, setShowAdd] = useState(false);
  const [editingSchedule, setEditingSchedule] = useState<ChainScheduleDto | null>(null);
  ...
  function openEdit(s: ChainScheduleDto) {
  ...
  function handleCreate() {
    const req: CreateChainScheduleRequest = {
      scheduleType,
      isEnabled,
  ...
  function handleUpdate() {
    ...
    const req: UpdateChainScheduleRequest = {
  ...
  function handleToggle(s: ChainScheduleDto) {
    updateSchedule.mutate(
      ...
      <Dialog open={showAdd} onOpenChange={(open) => { setShowAdd(open); if (!open) resetAddForm(); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Create a new schedule for this chain.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Switch checked={isEnabled} onCheckedChange={setIsEnabled} id="chain-add-enabled" />
              <Label htmlFor="chain-add-enabled">Enabled</Label>
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent>
          ...
          {editingSchedule && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <Switch checked={editEnabled} onCheckedChange={setEditEnabled} id="chain-edit-enabled" />
                <Label htmlFor="chain-edit-enabled">Enabled</Label>
              </div>
            </div>
          )}
          <DialogFooter>
